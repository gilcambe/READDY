...
async function* streamGroq(system, messages, modelId, maxTok) {
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: modelId || 'llama-3.3-70b-versatile', max_tokens: Math.min(maxTok || 8192, 8192), stream: true, temperature: 0.4, messages: [{ role: 'system', content: system }, ...messages] })
    });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('[CORTEX v16] ❌', 'Internal error', err.stack);
    const status = 'Internal error'?.includes('Limite') ? 429 : 'Internal error'?.includes('não permitid') ? 403 : 500;
    return { statusCode: status, headers, body: JSON.stringify({ error: 'Internal error' }) };
  }
};
